...
exports.createPost = async (userId, body, files) => {
    // Kiểm tra giới hạn 3 bài pending
...
exports.updatePost = async (userId, id, body, files) => {
    ...
    const updatedData = {
        ...
        price: Number(price),
        category: category || 'Khác',
        status: 'pending' // Sửa xong sẽ chờ duyệt lại
    };
...
exports.getPostById = async (id, user = null) => {
    ...
    // Nếu bài chưa được duyệt thì chỉ người bán hoặc admin mới được xem
    if (post.status !== 'approved') {
        if (!user) {
...
exports.purchasePost = async (buyerId, postId) => {
    ...
    const platformFee = Math.round(finalPrice * 0.05); // phí 5%
    ...
    // Nếu trừ tiền thành công, xử lý phần còn lại
    try {
